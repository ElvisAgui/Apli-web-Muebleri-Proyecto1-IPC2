...
public class Devolucion extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String pHat = escritor.escribirArchivo((ArrayList<Venta>) Controlador.listaV1, path, true);
        try {
            BufferedInputStream file = new BufferedInputStream(new FileInputStream(pHat));
            response.setContentType("text/csv;charset=UTF-8");
            response.setHeader("Content-disposition", "attachment; filename=ReporteDevolucion.csv");
            int data = file.read();
            while (data > -1) {
