 ...
 * @author elvis_agui
 */
public class GananciaUsuario extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String pHat = escritor.EscArchivoGananciaUS((ArrayList<Venta>) Controlador.listaV1, path, request.getParameter("Tganancia"), request.getParameter("usuario"));
        try {
            BufferedInputStream file = new BufferedInputStream(new FileInputStream(pHat));
            response.setContentType("text/csv;charset=UTF-8");
            response.setHeader("Content-disposition", "attachment; filename=ReporteGananciaUS.csv");
            int data = file.read();
            while (data > -1) {
