 ...
 * @author elvis_agui
 */
public class MenosV extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String pHat = escritor.EscArchivoMuebleMenos((ArrayList<Venta>) Controlador.listaV1, path, request.getParameter("mueble"));
        try {
            BufferedInputStream file = new BufferedInputStream(new FileInputStream(pHat));
            response.setContentType("text/csv;charset=UTF-8");
            response.setHeader("Content-disposition", "attachment; filename=ReporteMenoMueble.csv");
            int data = file.read();
            while (data > -1) {
