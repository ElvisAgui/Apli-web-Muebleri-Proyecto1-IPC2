 ...
 * @author elvis_agui
 */
public class Ganancia extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String pHat = escritor.EscArchivoGanancia((ArrayList<Venta>) Controlador.listaV1, path, request.getParameter("Tganancia"));
        try {
            BufferedInputStream file = new BufferedInputStream(new FileInputStream(pHat));
            response.setContentType("text/csv;charset=UTF-8");
            response.setHeader("Content-disposition", "attachment; filename=ReporteGanancia.csv");
            int data = file.read();
            while (data > -1) {
